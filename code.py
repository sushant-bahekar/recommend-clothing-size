...
#replace space with _ symbol
df.columns = df.columns.str.strip().str.lower().str.replace(' ','_')

missing_data = df.isnull().sum()*100.0/len(df)

X = df.drop(['fit','waist', 'bust', 'user_name','review_text','review_summary','shoe_size','shoe_width'],axis=1)
...
# Code starts here
def get_cms(x):
    if type(x) == type(1.0): #for null values
        return
...
# --------------
# Code starts here
X_train = pd.get_dummies(data=X_train, columns=
['category','cup_size','length'], prefix=['category','cup_size','length'])

X_test = pd.get_dummies(data=X_test, columns=
['category','cup_size','length'], prefix=['category','cup_size','length'])
